-- Project CMS compatibility migration.
--
-- This migration is safe to run against an existing project database:
-- it only creates missing objects/columns and recreates the intended policies.
-- It does not delete or rewrite existing project rows.
...
-- These statements make the migration safe for a partially-created gallery
-- table as well as for a database where the table did not exist yet.
alter table public.project_gallery_items
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.project_gallery_items'::regclass
      and contype = 'f'
      and confrelid = 'public.projects'::regclass
  ) then
    alter table public.project_gallery_items
      add constraint project_gallery_items_project_id_fkey
      foreign key (project_id) references public.projects(id) on delete cascade;
  end if;
end;
$$;

create unique index if not exists project_gallery_items_project_id_display_order_key
  on public.project_gallery_items (project_id, display_order);
...
-- Ask PostgREST to discard a stale schema cache after the DDL commits.
